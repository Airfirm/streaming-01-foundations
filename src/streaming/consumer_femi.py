"""src/streaming/consumer_femi.py - Modified local consumer example.

Reads messages from a local simulated topic file and writes consumed records
to a local output file one message at a time.

Modifications:
- Reads from:
  - data/output/streaming-01-foundations-femi.csv
- Adds consumer tracking fields:
  - consumed_at_utc
  - processing_status
- Writes consumed output file as:
...
# This must match the topic file created by producer_femi.py
TOPIC_CSV: Final[Path] = OUTPUT_DIR / "streaming-01-foundations-femi.csv"

# Modified consumer output file name
...
def verify_topic_file() -> None:
    ...
    while not TOPIC_CSV.exists():
        ...
        if elapsed >= TIMEOUT_SECONDS:
            ...
            LOG.error(
                "Run producer_femi.py first, or run producer and consumer together."
            )
            raise SystemExit(1)
...
def process_message(row: dict[str, Any]) -> dict[str, Any]:
    """Process one local message by adding consumer tracking fields.

    Arguments:
        row: A local message row.

    Returns:
        The processed row with consumer metadata.
    """
    LOG.info("Processing raw local message.")

    processed_row = dict(row)
    processed_row["consumed_at_utc"] = datetime.now(UTC).isoformat()
    processed_row["processing_status"] = "consumed"
