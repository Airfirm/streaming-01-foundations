"""src/streaming/producer_femi.py - Modified local producer example.

Reads sales from data/sales.csv and writes records to a local simulated topic
file one message at a time.

Modifications:
- Adds message tracking fields:
  - message_id
  - produced_at_utc
- Writes the simulated topic output file as:
  - data/output/streaming-01-foundations-femi.csv
...
Terminal command to run this file from the root project folder:
    ...
    uv run python -m streaming.consumer_femi
"""
...
# Modified producer topic output file name
TOPIC_CSV: Final[Path] = OUTPUT_DIR / "streaming-01-foundations-femi.csv"
...
def enrich_message(message: dict[str, Any], message_number: int) -> dict[str, Any]:
    """Add tracking fields to each produced message.

    Arguments:
        message: The original sales message.
        message_number: The current message number.

    Returns:
        The sales message with tracking fields added.
    """
    enriched_message = dict(message)
    enriched_message["message_id"] = f"sale-{message_number}"
    enriched_message["produced_at_utc"] = datetime.now(UTC).isoformat()
...
def send_messages() -> int:
    ...
    try:
        for message in generate_messages(MESSAGE_COUNT):
            ...
            sent_count += 1
            enriched_message = enrich_message(message, sent_count)

            send_local_message(enriched_message)

            LOG.info(f"  MESSAGE SENT  sent={sent_count}")
            LOG.info(f"  message_id={enriched_message['message_id']}")
